...
Data sources (all in SnTe_FF/Spectral/TDSCHA_Interpolate/):
  direct_444.dat              — direct 4x4x4 q-Lanczos (reference)
...
Palette (validated from make_figs.py):
  direct/reference  #2a78d6  blue
  atomic_delta      #d9361e  red (the new validated method)
  tensor-D3         #1baf7a  green-aqua
  atomic raw        #4a3aa7  violet (permuation-correct, no ASR)
  plain             #eda100  yellow
...
C_DIR = "#2a78d6"     # direct 4x4x4 reference
...
def load_data(filename):
    d = np.loadtxt(os.path.join(ROOT, filename))
...
e_bub, s_bub_all = load_data("old_gamma_noLOTO.dat")  # col 1 = TO, col 2 = total
...
# Determine common x-range and find peaks
sel = (e_dir > 15) & (e_dir < 70)
...
print("Peaks (cm-1):")
print("  direct 4x4x4:    %.2f" % peak(e_dir, s_dir))
...
def norm(s, e):
    return s / np.trapezoid(s, e)

# Direct 4x4x4 (reference, shaded background)
ax.fill_between(e_dir, norm(s_dir, e_dir), alpha=0.22, color=C_DIR, lw=0,
                label="direct $4^3$ q-Lanczos (reference)")
ax.plot(e_dir, norm(s_dir, e_dir), color=C_DIR, lw=2.6, alpha=0.7)
...
# tensor-D3 oracle
ax.plot(e_fix, norm(s_fix, e_fix), color=C_TENSOR, lw=1.6, ls="--",
...
# SSCHA harmonic position
ax.axvline(54.0, color="#777777", lw=1.0, ls="--")
ax.annotate(r"SSCHA $\omega^{\rm TO}_\Gamma=54.0$ cm$^{-1}$",
            xy=(54.0, 0.92), xytext=(57.0, 0.95),
...
# ---------------------------------------------------------------------------
# Second figure: zoom on the TO peak region (32-42 cm-1), normalized
# ---------------------------------------------------------------------------
fig2, ax2 = plt.subplots(figsize=(5.0, 3.0), constrained_layout=True)

# Direct 4x4x4 reference
ax2.fill_between(e_dir, norm(s_dir, e_dir), alpha=0.25, color=C_DIR, lw=0,
                 label="direct $4^3$ (reference)")
ax2.plot(e_dir, norm(s_dir, e_dir), color=C_DIR, lw=2.4, alpha=0.7)
...
# tensor-D3 oracle
ax2.plot(e_fix, norm(s_fix, e_fix), color=C_TENSOR, lw=1.6, ls="--",
...
# Mark peak positions
for ee, ss, label, color, yoff in [
        (e_dir, s_dir, "37.56", C_DIR, 0.88),
        (e_ad, s_ad, "37.52", C_DELTA, 0.78),
        (e_fix, s_fix, "37.65", C_TENSOR, 0.68),
        (e_bub, s_bub, "38.46", C_BUB, 0.58),
    ]:
    pk = peak(ee, ss, 32, 42)
...
ax2.legend(loc="upper left", fontsize=7.0, ncol=1)
