...
The production tensor-free fix is a pinned, atom-resolved kernel.  For one
primitive atom $a$ and one coarse origin $\mathbf{o}$, pin the perturbation
slot to
\begin{equation}
...
atomic, $N_I=1000$, 30 steps & 37.72 & 0.0865 & 0.0466\\
...
spurious stable $\approx 50\,\mathrm{cm}^{-1}$ response.  The atomic mode
therefore reaches the centered-tensor quality on the real-material test
without constructing or storing the third-order tensor.  Its current cost
is the number of pinned-atom/origin passes (SnTe: $2\times8=16$ D3 pass
calls); D4 remains on the single plain pass.
