...
\texttt{ForceTensor}. Finally, for $L=2$ real-material cells where
cell-index windows cannot break basis ties, we introduce atom-resolved
pinned windows that use only geometry. The permutation-correct version
(with three force-channel families and a conservative $q$-space acoustic
projector) recovers the SnTe $2^3\to4^3$ interpolated $\Gamma$ TO peak
(37.5~cm$^{-1}$ = the direct $4^3$ reference) while reducing the
near-$\Gamma$ acoustic $D_3$ vertex leakage by four orders of magnitude,
without ever constructing $\Phi^{(3)}$. We derive the
method, prove its equivalence with centered-tensor interpolation, validate
it against direct fine-supercell calculations on an anharmonic chain model,
and demonstrate agreement with the standard $d_3$ dynamic-bubble spectral
functions of \texttt{cellconstructor.Spectral} at interpolated wavevectors.
\end{abstract}
...
The cell-index windows above deliberately mimic the minimal-spread image
assignment in the \emph{supercell lattice}.  On the SnTe force-field
benchmark this is not enough.  The tutorial ensemble has a $2\times2\times2$
supercell: every nonzero cell difference is a Wigner--Seitz tie for a
one-period cell-index kernel, so the plain tent is the only admissible
assignment.  However \texttt{ForceTensor.Tensor3.Center(Far=3)} succeeds on
the same data because it makes a finer, geometry-dependent choice: it uses
the actual Cartesian interatomic vectors, including the basis offsets, and
minimizes the triplet perimeter over replicas.  The coarse data therefore
contains the needed information; the missing ingredient is atom-resolved
centering.

The tensor-free fix is a pinned, atom-resolved kernel.  For one primitive
atom $a$ and one coarse origin $\mathbf{o}$, first consider pinning the
perturbation slot to
\begin{equation}
...
the geometry-only atomic minimal images
\begin{equation}
...
\end{equation}
splitting exact ties uniformly.  The same window is used for the two
pair slots.  Summing over all origins restores the translation sum that
the delta-pinned $z$ window removes, so no $1/\Nc$ origin-averaging
prefactor is applied.  The realized D3 kernel is
\begin{equation}
...
At runtime an extended image outside the fundamental supercell is folded
onto the periodic atom and multiplied by the corresponding Bloch phase
...
The stochastic D3 estimator in the Julia implementation already represents
the permutation symmetry of $\Phi^{(3)}$ by splitting the three possible
force legs: one third of the estimator has the force on the perturbation
slot $z$, and the remaining two thirds have the force on the two pair slots
$w$ and $v$.  The atom-resolved implementation must follow the same split.
The production path therefore runs three channel families:
\begin{align}
...
\end{align}
For identical slot windows, the sum of the three channel calls is
algebraically identical to the original all-channel D3 kernel; this is now a
...
The same class-sum structure gives the strict-$\Gamma$ acoustic sum rule
without constructing the tensor.  For every folded atom/cell class, the
non-pinned atomic windows have total selected-image weight one.  After
summing all pinned atoms and all coarse origins, the pinned slot has the
same unit class sum.  Hence contracting an ASR-satisfying folded coarse
$\Phi^{(3)}$ with a uniform acoustic displacement at $q=0$ sees exactly the
same folded sums as the plain commensurate estimator, and vanishes.  What is
not automatic is the first moment of the image distribution: near $\Gamma$,
different centered images can change the $O(q)$ acoustic-vertex slope.
This is now treated as a diagnostic/correction problem rather than by
averaging away the pinned atom.  A low-rank prototype that used a uniform
pinned slot and an SVD of the atom-averaged pair kernel reduced SnTe to the
bad plain result (peak $\simeq52\,\mathrm{cm}^{-1}$), showing that the
atom-conditioned basis-offset information must remain in the leading
kernel.

Before using this in the stochastic estimator we performed an oracle test:
contract the \emph{periodic} SnTe $\Phi^{(3)}$ tensor with candidate
geometry kernels, but use the tensor only to measure the kernel quality.
...
The key result is that \texttt{atomic\_delta} retains the correct TO peak
...
sum rule. The acoustic-mode $D_3$ vertex norm at the smallest non-zero $q$ on
the $4^3$ mesh ($|\qv|\approx 0.066$~\AA$^{-1}$) is reduced from
...
calls); D4 remains on the single plain pass.  The coherent improvement in
both spectral accuracy and ASR compliance makes \texttt{atomic\_delta} the
recommended tensor-free production mode for underresolved small supercells
($L=2$) where cell-index window schemes fail to resolve atomic-basis ties.
...
The atom-resolved mode of Sec.~\ref{sec:atomic} has pass count
$n_{\rm at}\Nc$ (one pinned atom times all coarse origins; 16 for the SnTe
$2^3$ cell) and remains linear in $\Nf$, but with a larger constant than
the tensor-D3 oracle.
