...
This module is the single entry point for all the Julia calls of tdscha.
The Julia runtime is NOT booted when this module (or tdscha) is imported;
it is initialized on the first call to :func:`get_main`, so that users who do
not use ``MODE_FAST_JULIA`` never pay the startup cost.
...
tdscha Julia kernels use strictly-typed signatures that do not dispatch
...
# The Julia source files defining the tdscha kernels, included at first use.
_JL_FILES = ["tdscha_core.jl", "tdscha_qspace.jl"]
...
def get_main():
    ...
    The first call boots the Julia runtime and includes the tdscha Julia
