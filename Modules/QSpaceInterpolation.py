...
def interpolate_dyn_fine(dyn, q_points, use_asr=True, reuse_commensurate=True,
    ...
    ignore_effective_charges : bool
        Hide the Born effective charges and the dielectric tensor from
        ``ForceTensor.Tensor2`` *for this interpolation only*.  Default
        False, i.e. the stored Z*/eps are used, as for any other
        cellconstructor Fourier continuation.  ``dyn`` itself is never
        modified: the charges are stripped on a local copy, so anything
        downstream that needs them (IR intensities, Raman/IR response,
        LO-TO at Gamma) still sees them.

        Why the flag exists.  When ``dyn.effective_charges`` is set,
        ``SetupFromPhonons`` *subtracts* the Ewald dipole-dipole term
        (``symph.rgd_blk``, sign -1) from every commensurate block, centers
        and ASR-projects only the short-range remainder, and ``Interpolate``
        *adds the Ewald term back* (sign +1).  ``lo_to_splitting=False``
        does not disable this; it only suppresses the extra nonanalytic
        term exactly at Gamma.

        That cycle is the identity on the coarse mesh -- so it is invisible
        in every commensurate test -- but it is *not* the identity between
        coarse points: off-grid it replaces the plain Fourier continuation
        of the measured force constants with an analytic dipole
        continuation.  For a genuinely polar potential that is the right
        thing to do, and this is why it is the default.  But if the
        ensemble was generated by a potential with no intrinsic long-range
        electrostatics -- a short-range machine-learning interatomic
        potential, for instance -- then the Z*/eps in the
        dynamical-matrix header are inherited metadata, not a property of
        the potential that produced the forces.  Subtracting a dipole tail
        that is not in the data leaves a remainder that is *not*
        short-ranged, so centering truncates it and the ASR is imposed on
        the wrong object; the re-added Ewald term then does not cancel the
        truncation error anywhere except on the coarse mesh.  On CsSnI3
        this produced twelve spurious imaginary modes on the off-grid
        half-shell of the 8^3 mesh, all of which disappear with
        ``ignore_effective_charges=True``
        (``report/interpolation/scripts/cssni3_longrange_audit.py``).
    ...
    if dyn.effective_charges is not None and ignore_effective_charges:
        # Strip on a local copy: the caller's dyn keeps its charges, so IR /
        # Raman response functions downstream are unaffected.
        dyn = dyn.Copy()
        dyn.effective_charges = None
        dyn.dielectric_tensor = None
        uc = dyn.structure
    ...
    def __init__(self, ensemble, fine_mesh=None, use_asr_dyn=True,
        ...
        if d4_center not in (False, True, "leg", "reference", "factor"):
            raise ValueError("d4_center must be False, True, 'leg', "
                             "'reference', or 'factor'")
        if d4_center == "leg":
            # NOTE: no local "import warnings" here -- it would make the name
            # function-local and turn every later warnings.warn() in this
            # constructor into an UnboundLocalError.
            warnings.warn(
