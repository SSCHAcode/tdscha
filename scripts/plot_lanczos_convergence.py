...
def main():
    ...
    if not len(sys.argv) in [2, 3]:
        print("Error, I require the Lanczos .npz file (or the prefix of .abc file for the C++ code) to be analyzed, and (optionally) the smearing [cm-1]")
        raise ValueError("Error, I require the Lanczos .npz (or the prefix of the .abc) file to be analyzed")
    ...
    # Check if the file is a lanczos status or a cluster prefix
    if fname.endswith(".npz"):
        ...
        print("Extension .npz not found. Expected a cluster prefix.")
        print("Looking for the file {}.abc ... ".format(fname),)
        if not os.path.exists(fname + ".abc"):
            print("  Not found.")
            raise IOError("Error, no valid tdscha calculation with prefix {} found. Check if you mispelled it.".format(fname))
        else:
            print("  Found.")

        data.load_from_input_files(fname)
    ...
    print()
    print("The final static frequency is {} cm-1".format(w_static[-1]))
